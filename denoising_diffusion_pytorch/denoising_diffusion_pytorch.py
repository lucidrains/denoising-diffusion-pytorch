...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, condition, t, x_self_cond=None, clip_x_start=False):
        model_output = self.model(x, condition, t).unsqueeze(1)
    ...
    def p_mean_variance(self, x, condition, t, x_self_cond=None, clip_denoised=True):
        preds = self.model_predictions(x, condition, t, x_self_cond)
    ...
    def p_sample(self, x, condition, t: int, x_self_cond=None):
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x=x, condition=condition, t=batched_times,
    ...
    def p_sample_loop(self, condition, shape, return_all_timesteps=False):
        ...
        for t in reversed(range(0, self.num_timesteps)):
            ...
            img, x_start = self.p_sample(img, condition, t, self_cond)
            imgs.append(img)
    ...
    def sample(self, condition, batch_size=16, return_all_timesteps=False):
        ...
        return sample_fn(condition, (batch_size, image_size[0], image_size[1]),
    ...
    def p_losses(self, x_start, condition, t, noise=None):
        ...
        model_out = self.model(x, condition, t).unsqueeze(1)
        ...
        loss = self.loss_fn(model_out, target, reduction='none').squeeze(1)
    ...
    def forward(self, img, condition, *args, **kwargs):
        b, h, w, device, img_size, = *img.shape, img.device, self.image_size
